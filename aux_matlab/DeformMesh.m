function [defMesh] = DeformMesh(mesh,X2,Y2,fixSquareDomain)
    ...
    if isempty(Y2)
        ...
        Dx_new  = diag(x2p.^-1)*Dx;
        D2x_new = diag(x2p.^-2)*D2x + diag(x2pp.*x2p^-3)*Dx ;
    ...
    elseif isempty(X2)
        ...
        if y_symmetry
            Dy_symm_new  = diag(y2p.^-1)*Dy_symm;
            D2y_symm_new = diag(y2p.^-2)*D2y_symm+ diag(y2pp.*y2p^-3)*Dy_symm;        

            Dy_asymm_new  = diag(y2p.^-1)*Dy_asymm;
            D2y_asymm_new = diag(y2p.^-2)*D2y_asymm+ diag(y2pp.*y2p^-3)*Dy_asymm;        
